...
<%
    ...
    String orderClause = PstAppPriv.fieldNames[PstAppPriv.FLD_PRIV_ID];//untuk menyimpan orderClause dalam proses menampilkan data dalam list tabel
    ...
    long IDAppPriv = 0; //untuk menyimpan ID objek yang sedang dimaintenance (0: untuk data baru)
    ...
    if (isMultipart) {
        ...
        while (iter.hasNext()) {
            ...
            if (item.isFormField()) { //untuk membaca inputan user yang bukan file dengan item.getFieldName() dan item.getString()
                ...
                else if (item.getFieldName().matches("hidden_apppriv_id")) IDAppPriv = Long.parseLong(item.getString());
                else {
    ...
    CtrlAppPriv ctrlAppPriv = new CtrlAppPriv(paramValues); //memanggil control untuk melakukan manipulasi data dengan mengirim Hashtable yang sudah ada nilai inputan user
    ExtendedControlLine ctrLine = new ExtendedControlLine(); //membuat sebuah control line untuk tempat command
    Vector listAppPriv = new Vector(1,1); //membuat sebuah vector yang berisikan data yang akan ditampilkan dalam list tabel
    String whereClause = ""; //untuk menyimpan whereClause dalam proses menampilkan data dalam list tabel
    
    /*switch statement */
    iErrCode = ctrlAppPriv.action(iCommand, IDAppPriv, imageDirectory); //melakukan proses dengan mengirimkan command yang dilakukan, IDAppPriv, dan folder directory tempat menyimpa file, dan mengembalikan kode error
    /* end switch*/

    FrmAppPriv frmAppPriv = ctrlAppPriv.getForm(); //mengambil form dari control untuk proses menampilkan tampilan berikutnya

    /*count list All Product*/
    int vectSize = PstAppPriv.getCount(whereClause); //mengambil jumlah data yang tersedia
    AppPriv apppriv = ctrlAppPriv.getAppPriv(); //mengambil objek apppriv yang sedang dimaintenance
    msgString =  ctrlAppPriv.getMessage(); //mengambil nilai message utama dalam proses manipulasi data

    
    /*switch list AppPriv*/ //proses untuk mendapatkan nilai start dari tabel yang akan ditampilkan
    if((iCommand == Command.SAVE) && (iErrCode == ExtendedFRMMessage.NONE)&& (IDAppPriv == 0))
	start = PstAppPriv.findLimitStart(apppriv.getOID(), recordToGet, whereClause, orderClause);

    if((iCommand == Command.FIRST || iCommand == Command.PREV )||
        (iCommand == Command.NEXT || iCommand == Command.LAST)){
        start = ctrlAppPriv.actionList(iCommand, start, vectSize, recordToGet);
    }
    /* end switch list*/

    /* get record to display */ //mendapatkan list data yang akan ditampilkan
    listAppPriv = PstAppPriv.list(start, recordToGet, whereClause, orderClause);

    /*handle condition if size of record to display = 0 and start > 0 	after delete*/
    if (listAppPriv.size() < 1 && start > 0){
    ...
    else if (iCommand == Command.SAVE && frmAppPriv.errorSize()>0) statusInput = 9;

%>

<%@ include file = "../session.jsp"%> 
<%
if(privPrivileges==false){
%>
<script>
...
<script language="JavaScript">
                function cmdAdd(){
                    document.frmapppriv.hidden_apppriv_id.value="0";
                    document.frmapppriv.command.value="<%=Command.ADD%>";
                    document.frmapppriv.prev_command.value="<%=prevCommand%>";
                    document.frmapppriv.action="index.jsp?page=user/apppriv.jsp";
                    document.frmapppriv.submit();
                }
                function cmdDelete(IDAppPriv){
                    document.frmapppriv.hidden_apppriv_id.value=IDAppPriv;
...
<form role="form" name="frmapppriv" method ="post" action="" enctype="multipart/form-data">
<input type="hidden" name="command" value="<%=iCommand%>">
<input type="hidden" name="vectSize" value="<%=vectSize%>">
<input type="hidden" name="start" value="<%=start%>">
<input type="hidden" name="prev_command" value="<%=prevCommand%>">
<input type="hidden" name="hidden_apppriv_id" value="<%=IDAppPriv%>">  
      <table width="100%" border="0">
        <tr>
          <td><table width="100%" border="0">
            <tr>
              <td bgcolor="#CCCCCC">No</td>
              <td bgcolor="#CCCCCC">Nama Privilege</td>
              <td bgcolor="#CCCCCC">Deskripsi</td>
            </tr>
            <%
            for(int i=0; i<listAppPriv.size(); i++){
                AppPriv view = (AppPriv)listAppPriv.get(i);
            %>
            <tr>
              <td><%=i+start+1%></td>
              <td><a href="javascript:cmdEdit('<%=view.getOID()%>')"><%=view.getPrivName()%></a></td>
        ...
        <tr>
          <td>
              <%if((iCommand==Command.ADD)||(iCommand==Command.EDIT)||(frmAppPriv.errorSize()>0)){%>
              <table width="100%" border="0">
            <tr>
              <td colspan="2" bgcolor="#CCCCCC">Form Data AppPriv</td>
            </tr>
            <tr>
              <td>Nama Privilege</td>
              <td><label>
                <input type="text" name="<%=frmAppPriv.fieldNames[FrmAppPriv.FRM_PRIV_NAME]%>" id="textfield" value="<%=apppriv.getPrivName()%>"/>
              </label></td>
            </tr>
            <tr>
              <td>Deskripsi</td>
              <td><label>
                <input type="text" name="<%=frmAppPriv.fieldNames[FrmAppPriv.FRM_DESCRIPTION]%>" id="textfield2" value="<%=apppriv.getDescription()%>"/>
              </label></td>
            </tr>
            <tr>
              <td colspan="2">
                ...
                <%if(IDAppPriv!=0){%>
                <input onclick="javascript:cmdDelete('<%=IDAppPriv%>')" type="submit" name="button3" id="button3" value="Delete">
              <%}%>
                  </td>
            </tr>
          </table>
